...
statistics. Each room gives you a story, a task, and a lock. You solve the task
...
Load the package and call `escape()`:
...
Text answers can be submitted as character strings:
...
This tells you the active player, how many rooms have been solved, and which
room is current. It is useful during longer activities or when returning to the
game after a break.
...
`escapeR_file()` to find them:

```{r}
escapeR_file("dados1.csv")
```

For example, a room might ask you to read a CSV file:

```{r, eval = FALSE}
d <- read.csv(escapeR_file("dados1.csv"))
nrow(d)
```

The package also provides a small survey data set:

```{r}
head(survey_counts())
```

This data set appears in several rooms about data quality, summaries, modelling,
...
The `id` column is useful when building a shorter custom quest.

## Play a shorter or custom quest

Instructors can build a quest from selected room IDs:
